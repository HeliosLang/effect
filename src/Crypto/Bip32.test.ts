...
import { Bytes, Utf8 } from "../Codecs/index.js"
import * as Bip32 from "./Bip32.js"

describe("Crypto.Bip32.sign", () => {
  it('correctly signs "Hello World"', () => {
    ...
    const key = Effect.runSync(Bip32.makeSigningKey(bytes))
    expect(Bytes.toArray(Bip32.skBytes(key))).toEqual(bytes)

    const message = Utf8.encode("Hello World")

    const expectedSignature = [
      0x90, 0x19, 0x4d, 0x57, 0xcd, 0xe4, 0xfd, 0xad, 0xd0, 0x1e, 0xb7, 0xcf,
      0x16, 0x17, 0x80, 0xc2, 0x77, 0xe1, 0x29, 0xfc, 0x71, 0x35, 0xb9, 0x77,
      0x79, 0xa3, 0x26, 0x88, 0x37, 0xe4, 0xcd, 0x2e, 0x94, 0x44, 0xb9, 0xbb,
      0x91, 0xc0, 0xe8, 0x4d, 0x23, 0xbb, 0xa8, 0x70, 0xdf, 0x3c, 0x4b, 0xda,
      0x91, 0xa1, 0x10, 0xef, 0x73, 0x56, 0x38, 0xfa, 0x7a, 0x34, 0xea, 0x20,
      0x46, 0xd4, 0xbe, 0x04
    ]

    const signature = Bytes.toArray(Bip32.sign(key)(message).bytes)
