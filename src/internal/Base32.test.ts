...
describe("Base32.decode()", () => {
    ...
    it('returns the utf-8 bytes of "f" for "my"', () => {
        expect(paddingLessCodec.decode("my")).toEqual(Either.right(Utf8.encode("f")))
    })

    it('returns the utf-8 bytes of "fo" for "mzxq"', () => {
        expect(Base32.DEFAULT.decode("mzxq")).toEqual(Either.right(Utf8.encode("fo")))
    })
    ...
    it('returns the utf-8 btyes of "foo" for "mzxw6"', () => {
        expect(Base32.DEFAULT.decode("mzxw6")).toEqual(Either.right(Utf8.encode("foo")))
    })

    it('returns the utf-8 bytes of "foob" for "mzxw6yq"', () => {
        expect(Base32.DEFAULT.decode("mzxw6yq")).toEqual(Either.right(Utf8.encode("foob")))
    })

    it('returns the utf-8 bytes of "fooba" for "mzxw6ytb"', () => {
        expect(Base32.DEFAULT.decode("mzxw6ytb")).toEqual(Either.right(Utf8.encode("fooba")))
    })

    it('returns the utf-8 bytes of "foobar" for "mzxw6ytboi"', () => {
        expect(Base32.DEFAULT.decode("mzxw6ytboi")).toEqual(Either.right(Utf8.encode("foobar")))
    })
    ...
    it('returns the same for "mzxw6ytboi" as for "MZXW6YTBOI" (case insensitive)', () => {
        ...
        expect(Base32.DEFAULT.decode(s)).toEqual(Base32.DEFAULT.decode(s.toUpperCase()))
    })
})
