...
describe("Bytes.makeArray()", () => {
    it("converts [255] into [255]", () => {
        expect(Bytes.makeArray([255])).toEqual([255])
    })

    it("converts #ff into [255]", () => {
        expect(Bytes.makeArray("ff")).toEqual([255])
    })

    it("converts Uint8Array([255]) into [255]", () => {
        expect(Bytes.makeArray(new Uint8Array([255]))).toEqual([255])
    })

    it("converts ByteStream([255]) into [255]", () => {
        expect(Bytes.makeArray(Bytes.makeStream({ bytes: [255] }))).toEqual([255])
    })

    it("converts ByteStream([255]) into [255]", () => {
        expect(Bytes.makeArray(Bytes.makeStream([255]))).toEqual([255])
    })

    it("fails for wrong type", () => {
        expect(
            () => Bytes.makeArray({} as any)
        ).toThrow()
    })
})

describe("Bytes.makeUint8Array()", () => {
    it("returns [] for #", () => {
        expect(Bytes.makeUint8Array("")).toEqual(new Uint8Array([]))
    })
    ...
    it("returns [] for empty Uint8Array", () => {
        expect(Bytes.makeUint8Array(new Uint8Array([]))).toEqual(new Uint8Array([]))
    })

    it("returns [255] for #ff", () => {
        expect(Bytes.makeUint8Array("ff")).toEqual(new Uint8Array([255]))
    })

    it("returns [255] for [255]", () => {
        expect(Bytes.makeUint8Array([255])).toEqual(new Uint8Array([255]))
    })

    it("returns [0] for [256]", () => {
        expect(Bytes.makeUint8Array([256])).toEqual(new Uint8Array([0]))
    })

    it("returns [255] for Uint8Array([255])", () => {
        expect(Bytes.makeUint8Array(new Uint8Array([255]))).toEqual(new Uint8Array([255]))
    })
})
...
describe("Bytes.Stream", () => {
    ...
    describe("typecheck of makeByteStream", () => {
        it("must be able to pass BytesLike to makeByteStream", () => {
            const bytes: string | number[] | Uint8Array | Bytes.Stream = /** @type {any} */ ([])
...
describe("Bytes.compare", () => {
    ...
    it("returns 1 when comparing #02020202 to #02010202", () => {
        expect(
            Bytes.compare("02020202", "02010202")
        ).toBe(1)
    })

    it("returns 1 when comparing #01010101 to #020202 with shortestFirst=true", () => {
        expect(
            Bytes.compare([0x01, 0x01, 0x01, 0x01], Bytes.makeArray("020202"), true),
        ).toBe(
            1
        )
    })
    ...
    it("returns 1 when comparing #010101 to #02020202 with shortestFirst=true", () => {
        expect(
            Bytes.compare(Bytes.makeArray("010101"), Bytes.makeUint8Array("02020202"), true),
        ).toBe(
            -1
        )
    })
    ...
    it("returns 0 when comparing #01010101 to #01010101", () => {
        expect(
            Bytes.compare(Bytes.makeArray("01010101"), "01010101"),
        ).toBe(
            0
        )
    })
    ...
    it("returns 1 when comparing #01010101 to #010101", () => {
        expect(
            Bytes.compare("01010101", "010101")).toBe(
            1
        )
    })
    ...
    it("returns 1 when comparing #010101 to #01010101", () => {
        expect(
            Bytes.compare(Bytes.makeUint8Array("010101"), Bytes.makeStream("01010101")),
        ).toBe(
            -1
        )
    })
...
describe("Bytes.dummy", () => {
    it("returns all 0 with default 2nd arg", () => {
        expect(Bytes.dummy(28)).toEqual(new Array(28).fill(0))
    })
})
...
describe("Bytes.equals", () => {
    ...
    it("returns false for Uint8Array([0]) and Uint8Array([1])", () => {
        expect(
            Bytes.equals(new Uint8Array([0]), new Uint8Array([1]))
        ).toBe(
            false
        )
    })
