    ...
    decodeRaw(encoded: string): Either.Either<number[], Encoding.DecodeException> {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (c == this.padChar) {
                ...
                return Either.left(Encoding.DecodeException(encoded, `Unexpected padding character '${c}' at position ${i}`))
            }

            const code = this.alphabet.indexOf(c.toLowerCase())

            if (code < 0) {
                return Either.left(Encoding.DecodeException(encoded, `Invalid base32 character '${c}' at position ${i}`))
            } else if (i == n - 1) {
                const nBitsExtra = n * 5 - Math.floor((n * 5) / 8) * 8

                if ((((1 << nBitsExtra) - 1) & code) != 0) {
                    return Either.left(Encoding.DecodeException(encoded, `Invalid base32 final character '${c}'`))
                }
            }
 ...
 * @returns
 */
function trim(encoded: string, padChar: string, strict: boolean): Either.Either<string, Encoding.DecodeException> {
    if (padChar.length == 1) {
        let n = encoded.length
        ...
        if (
            (strict || n < encoded.length) &&
            encoded.length % 8 != 0
        ) {
            return Either.left(Encoding.DecodeException(encoded, "Invalid length (expected multiple of 8)"))
        }
        ...
        if (nPad != 0) {
            if (nPad != 6 && nPad != 4 && nPad != 3 && nPad != 1) {
                return Either.left(Encoding.DecodeException(encoded, "Invalid number of base32 padding characters"))
            }
        }
