...
export const decode = (bytes: Bytes.BytesLike): Cbor.DecodeResult<Redeemer> => Either.gen(function* () {
    ...
    switch(tag) {
        ...
        default:
            return yield* Either.left(new Cbor.DecodeError(Bytes.makeStream(bytes), `unhandled Redeemer tag ${tag}`))
    }
})
