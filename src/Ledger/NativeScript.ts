...
export const All = Schema.Struct({
    ...
    scripts: Schema.Array(Schema.suspend((): Schema.Schema<NativeScript, NativeScriptJSON> => NativeScript))
})
...
export const decode = (bytes: Bytes.BytesLike): Cbor.DecodeResult<NativeScript> => Either.gen(function* () {
    ...
    switch (tag) {
        ...
        default:
            return yield* Either.left(new Cbor.DecodeError(stream, `unexpected NativeScript tag ${tag}`))
    }
})
...
export function validate(ctx: {
    ...
    return (script: NativeScript): boolean => {
        switch (script.type) {
            ...
            case "atLeast":
                return script.scripts.map(validate(ctx)).reduce((prev: number, b: boolean) => prev + Number(b), 0) >= script.required
            case "after":
