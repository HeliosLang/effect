...
export const Body = Schema.Struct({
    inputs: Schema.Array(UTxO.UTxO), // what is an intelligent way to handle a minimal Tx with onl
})
...
export const Witnesses = Schema.Struct({
    ...
    encoding: Schema.optional(Schema.Struct({
...
export type MetadataAttr = string | number | readonly MetadataAttr[] | Readonly<{[key: string]: MetadataAttr}>
...
export const MetadataAttr: Schema.Schema<MetadataAttr> = Schema.Union(
    ...
    Schema.Record({ // the standard permits maps with arbitrary keys, but limiting this to string keys is probably good enough for now
        key: Schema.String,
...
export function encodeWitnesses(witnesses: Witnesses): number[] {
    ...
    if (witnesses.signatures.length > 0) {
        ...
        m.set(
            0,
            encodeAsSet ? Cbor.encodeSet(encodedItems) : Cbor.encodeDefList(encodedItems)
        )
    }
