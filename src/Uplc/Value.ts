...
import { toArray, toUint8Array } from "../internal/Bytes.js"
import * as Flat from "../internal/Flat.js"
import * as Utf8 from "../internal/Utf8.js"
import * as ZigZag from "../internal/ZigZag.js"
import * as Data from "./Data.js"
import * as Type from "./Type.js"

// these value Schemas don't encode to JSON

const SuspendedValue = Schema.suspend(
  (): Schema.Schema<Value, Value> => Value
)
...
export const Bls12_381_G1Element = Schema.Struct({
  g1Element: Schema.Tuple(Schema.BigIntFromSelf, Schema.BigIntFromSelf, Schema.BigIntFromSelf)
})
...
export function flatSize(v: Value): number {
  if (typeof v == "boolean") {
    ...
    return 10 + nItemTypeBits + v.items.reduce((prev, item) => flatSize(item) - nItemTypeBits + prev, 0)
  } else {
    throw new Error(`unhandled value kind in Uplc.Value.flatSize() (got: ${v as unknown})`)
  }
}
...
export function memSize(v: Value): number {
  if (typeof v == "boolean" || v === null) {
    ...
    throw new Error(`unhandled value kind in Uplc.Value.memSize() (got: ${v as unknown})`)
  }
}
 ...
 * @param w 
 */
export const toFlat = (w: Flat.Writer) => (v: Value): void => {
  if (typeof v == "boolean") {
    w.writeBool(v)
  } else if (v === null) {
    ...
    throw new Error(`unhandled value kind in Uplc.Value.toFlat() (got: ${v as unknown})`)
  }
}
 ...
 */
const makeTypedDecoder = (r: Flat.Reader, typeList: number[]): Either.Either<ValueDecoder, TypeDecodingError> => Either.gen(function* () {
  ...
  switch (type) {
    ...
    case 2: // utf-8 string
      return () => Utf8.decode(r.readBytes()).pipe(Either.mapLeft((e) => new Error(e.message)))
    case 3: // unit
    ...
    case 5:
    case 6:
      return yield* Either.left(new Error("unexpected type tag without type application"))
    case 7: {
      const containerType = typeList.shift()

      if (containerType === undefined) {
        return yield* Either.left(new Error("expected nested type for container"))
      }
      ...
      switch (containerType) {
        ...
        case 7: {
          ...
          if (nestedContainerType === undefined) {
            ...
            return yield* Either.left(new Error(`unexpected nested container type tag (expected 6, got ${nestedContainerType})`))
          }
          ...
          return () => Either.zipWith(firstReader(), secondReader(), (first, second) => ({first, second}))
        }
        default: 
          return yield* Either.left(new Error(`unexpected container type tag ${containerType}`)) 
      }
    }
    case 8:
      return () => Data.decode(r.readBytes()).pipe(
        Either.map((d) => ({data: d})),
        Either.mapLeft((e) => new Error(e.message))
      )
    case 9:
      return yield* Either.left(new Error("Bls12_381_G1Element can't be deserialized"))
    case 10:
      return yield* Either.left(new Error("Bls12_381_G2Element can't be deserialized"))
    case 11:
      return yield* Either.left(new Error("Bls12_381_MlResult can't be deserialized"))
    default:
      return yield* Either.left(new Error(`unhandled value type ${type.toString()}`))
  }
})

function makeTypedListDecoder(r: Flat.Reader, itemTypeList: number[]): Either.Either<ValueDecoder, TypeDecodingError> {
  const itemTypeParts = Either.all(itemTypeList.map(x => Bits.fromByte(x, 4, false)))
 ...
 * @param r 
 */
export const decode = (r: Flat.Reader): Either.Either<Value, Error> => Either.gen(function* () {
...
export function toString(v: Value): string {
  if (typeof v == "boolean") {
    ...
    throw new Error(`unhandled value kind in Uplc.Value.toString() (got: ${v as unknown})`)
  }
}
...
export function toType(v: Value): Type.Type {
  if (typeof v == "boolean") {
    ...
    throw new Error(`unhandled value kind in Uplc.Value.toType() (got: ${v as unknown})`)
  }
}